/*
 * Orizon OS - Virtio MMIO Mouse/Tablet Driver
 *
 * QEMU virt input implementation for Orizon OS.
  ...
  while (kbd_last_used_idx != current_used) {
    ...
    if (ev->type == EV_KEY) {
      ...
      if (ev->value == 1) { /* Key press only */
        ...
        if (ev->code == 103)
          orizon_key = 0x100; /* KEY_UP */
        else if (ev->code == 108)
          orizon_key = 0x101; /* KEY_DOWN */
        else if (ev->code == 105)
          orizon_key = 0x102; /* KEY_LEFT */
        else if (ev->code == 106)
          orizon_key = 0x103; /* KEY_RIGHT */
        ...
        if (orizon_key) {
          if (key_callback)
            key_callback(orizon_key);
          if (gui_key_callback)
            gui_key_callback(orizon_key);
          processed = 1;
        }
