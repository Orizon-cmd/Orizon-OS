...
void term_execute_command(struct terminal *term, const char *cmd) {
  ...
  if (str_starts_with(cmd, "clear")) {
    ...
    term_puts(term, "\033[1;36mOrizon OS Terminal v2.0\033[0m\n");
    ...
    term_puts(term, "Orizon OS 0.5.0 ARM64 aarch64\n");
    ...
    term_puts(term, "\033[0m\n");
    term_puts(term, "\033[33mOS:\033[0m      Orizon OS 0.5.0\n");
    ...
    if (str_starts_with(topic, "nanoc") || str_starts_with(topic, "nano")) {
      ...
      term_puts(term, "Cross-compile C++ for Orizon OS using:\n");
  ...
  /* ===============================  */
    ...
    term_puts(term,
              "<body><h1>Hello from Orizon OS Network!</h1></body></html>\n");
  } else {
...
void term_handle_key(struct terminal *term, int key) {
  ...
  if (key == '\n' || key == '\r') {
    ...
    /* Show new prompt */
    term_puts(term, "\033[32morizon-os\033[0m:\033[34m~\033[0m$ ");
...
struct terminal *term_create(int x, int y, int cols, int rows) {
  ...
  term_puts(term, "\033[1;36mOrizon OS Terminal v1.0\033[0m\n");
