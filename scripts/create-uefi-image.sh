...
IMAGE_DIR="${2:-image}"
IMAGE_NAME="orizonos-x86_64.img"
IMAGE_SIZE="100M"
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    diskutil partitionDisk "$DISK" GPT FAT32 EFI 100M 2>/dev/null || {
        ...
        diskutil eraseDisk GPT OrizonOS "$DISK"
    }
    ...
    if [ -f "$BUILD_DIR/kernel/orizonos-x86_64.elf" ]; then
        cp "$BUILD_DIR/kernel/orizonos-x86_64.elf" "$EFI_MOUNT/EFI/BOOT/BOOTX64.EFI"
        log "Copied kernel as BOOTX64.EFI"
    else
        log "Kernel not found, creating placeholder..."
        echo "Orizon OS x86_64 kernel placeholder" > "$EFI_MOUNT/EFI/BOOT/README.txt"
    fi
