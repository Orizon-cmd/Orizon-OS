    ...
    *,
    ...
    run_sudo_command(
        ...
        + repr(
            ...
            f"NEED_INIT=0; "
            f"if [ ! -f {remote_disk} ]; then NEED_INIT=1; "
            f"elif ! parted -s {remote_disk} print 2>/dev/null | grep -q orizon-data; then NEED_INIT=1; fi; "
            f"if [ \"$NEED_INIT\" = 1 ]; then "
            f"truncate -s {disk_size} {remote_disk}; "
            f"parted -s {remote_disk} mklabel gpt; "
            f"parted -s {remote_disk} mkpart ESP fat32 1MiB 512MiB; "
            f"parted -s {remote_disk} set 1 esp on; "
            f"parted -s {remote_disk} mkpart orizon-data 512MiB 100%; "
            f"fi; "
