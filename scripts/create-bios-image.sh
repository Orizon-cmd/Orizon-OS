...
# Write kernel
if [ -f "$BUILD_DIR/kernel/orizonos-x86.elf" ]; then
    # Write kernel starting at sector 32 (16KB offset)
    dd if="$BUILD_DIR/kernel/orizonos-x86.elf" of="$IMAGE_PATH" bs=512 seek=32 conv=notrunc 2>/dev/null
