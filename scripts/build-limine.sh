#!/bin/bash
# Build Orizon OS for Limine bootloader and create bootable USB
set -e

cd "$(dirname "$0")/.."

echo "=== Building Orizon OS with Limine bootloader ==="
...
$LD $LDFLAGS -o build/limine/orizonos-limine.elf $(find build/limine -name '*.o')

echo "[INFO] Kernel built: build/limine/orizonos-limine.elf"
ls -lh build/limine/orizonos-limine.elf
