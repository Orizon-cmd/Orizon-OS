...
static inline void orizon_print_uint(kapi_t *k, unsigned int n) {
    if (n == 0) {
        orizon_putc(k, '0');
        return;
    }
...
static inline void orizon_print_hex(kapi_t *k, uint32_t n) {
    if (n == 0) {
        orizon_putc(k, '0');
        return;
    }
...
static inline void orizon_print_size(kapi_t *k, uint64_t bytes) {
    if (bytes < 1024) {
        orizon_print_uint(k, (unsigned int)bytes);
        orizon_puts(k, " B");
    } else if (bytes < 1024 * 1024) {
        orizon_print_uint(k, (unsigned int)(bytes / 1024));
        orizon_puts(k, " KB");
    } else if (bytes < 1024ULL * 1024 * 1024) {
        orizon_print_uint(k, (unsigned int)(bytes / (1024 * 1024)));
        orizon_puts(k, " MB");
    } else {
        orizon_print_uint(k, (unsigned int)(bytes / (1024ULL * 1024 * 1024)));
        orizon_puts(k, " GB");
    }
}
