...
/* Map OrizonOS key to DOOM key */
static unsigned char translate_key(int orizon_key) {
    /* Arrow keys */
    if (orizon_key == 0x100) return KEY_UPARROW;     /* KEY_UP */
    if (orizon_key == 0x101) return KEY_DOWNARROW;   /* KEY_DOWN */
    if (orizon_key == 0x102) return KEY_LEFTARROW;   /* KEY_LEFT */
    if (orizon_key == 0x103) return KEY_RIGHTARROW;  /* KEY_RIGHT */

    /* Modifier keys */
    if (orizon_key == 0x109) return KEY_RCTRL;       /* SPECIAL_KEY_CTRL = fire */
    if (orizon_key == 0x10A) return KEY_RSHIFT;      /* SPECIAL_KEY_SHIFT = run */

    /* Special keys */
    if (orizon_key == 27) return KEY_ESCAPE;
    if (orizon_key == '\n' || orizon_key == '\r') return KEY_ENTER;
    if (orizon_key == '\t') return KEY_TAB;
    if (orizon_key == ' ') return KEY_USE;           /* Space = use/open doors */
    if (orizon_key == 127 || orizon_key == 8) return KEY_BACKSPACE;

    /* Control key (ASCII 1-26 are Ctrl+letter) - also fire */
    if (orizon_key >= 1 && orizon_key <= 26) {
        return KEY_RCTRL;  /* Ctrl+letter = fire */
    }

    /* Function keys (if supported) */
    if (orizon_key >= 0x110 && orizon_key <= 0x11B) {
        return KEY_F1 + (orizon_key - 0x110);
    }

    /* WASD movement + E for use */
    if (orizon_key == 'w' || orizon_key == 'W') return KEY_UPARROW;
    if (orizon_key == 's' || orizon_key == 'S') return KEY_DOWNARROW;
    if (orizon_key == 'a' || orizon_key == 'A') return KEY_STRAFE_L;
    if (orizon_key == 'd' || orizon_key == 'D') return KEY_STRAFE_R;
    if (orizon_key == 'e' || orizon_key == 'E') return KEY_USE;

    /* Letters - lowercase them for DOOM */
    if (orizon_key >= 'A' && orizon_key <= 'Z') {
        return orizon_key + 32;  /* lowercase */
    }
    if (orizon_key >= 'a' && orizon_key <= 'z') {
        return orizon_key;
    }
    ...
    if (orizon_key >= '0' && orizon_key <= '9') return orizon_key;
    if (orizon_key == '-') return KEY_MINUS;
    if (orizon_key == '=') return KEY_EQUALS;
    if (orizon_key == '+') return '+';
    if (orizon_key == ',') return ',';
    if (orizon_key == '.') return '.';
    if (orizon_key == '/') return '/';

    /* Y/N for prompts */
    if (orizon_key == 'y' || orizon_key == 'Y') return 'y';
    if (orizon_key == 'n' || orizon_key == 'N') return 'n';
...
static void poll_keys(void) {
     ...
     * (OrizonOS doesn't have key-up events, so we fake them) */
...
void DG_Init(void) {
    ...
    printf("DG_Init: OrizonOS DOOM initialized (FULLSCREEN 2x)\n");
...
uint32_t DG_GetTicksMs(void) {
    /* OrizonOS ticks are 10ms each (100Hz timer) */
    uint64_t now = doom_kapi->get_uptime_ticks();
...
void DG_SetWindowTitle(const char *title) {
    /* No window title in OrizonOS - just print to console */
    (void)title;
}
