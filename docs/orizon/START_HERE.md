...
- Desktop release boundary for current package `0.84.0`: `desktop architecture`,
...
- `0.84.0` also adds the `lastDispatch` compositor diagnostic to layout
  state/tree and JSON dispatch output so VM operators can see the latest
  dispatcher, args, status, result text, workspace/layout/ratio state, submap,
  and focused client after a tiling action succeeds or fails.
