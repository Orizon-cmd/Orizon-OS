...
- Optional desktop profile foundation: `desktop status/config/config doctor/config apply/doctor/logs/keymap`,
  ...
  compact VM-safe `desktop hyprctl -j clients|workspaces|activeworkspace|activewindow|focushistory|workspacestack|clientmodel|rulematches|layoutstate|layouttree|monitors|configerrors|configtrace|getoption|keyword|reload|binds|layers` JSON for future tooling,
