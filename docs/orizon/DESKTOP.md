...
the effective mapped value, so `env`, `workspace`, `monitor`, `bind`,
`windowrulev2`, `layerrule`, and `source` imports are inspectable in ZimaOS VM
without claiming upstream Hyprland IPC, Wayland/wlroots, Waybar, floating/manual
...
reapplies the desktop policy after init during VM boot.
