...
hints under `/system`.
The desktop package version `0.98.0` installs explicit source/bind diagnostics,
...
areas, tiling area, scale policy, frame budget, rendered client counts,
...
including status, stable `error`/`hint`, and readable result text. The same
...
records the VM-safe `desktop-restore` reload for the Hyprland-style session and
tries `desktop recover` before recommending `desktop rescue` when reload warns.
