...
`desktop logs` or claiming upstream Hyprland IPC, Wayland/wlroots, Waybar
activation, floating/manual drag, or physical hardware validation. Version
`0.100.0` adds structured JSON reload diagnostics to
`desktop hyprctl -j reload` and generated package metadata through
`hyprctl-json-reload-structured`, so parser/source counts, runtime files,
session policy, and render/input settings can be checked in VM automation
without scraping prose or claiming upstream Hyprland IPC, Wayland/wlroots,
Waybar activation, floating/manual drag, or physical hardware validation.
Version `0.99.0` adds `keyword` route/effect diagnostics to
...
claiming upstream Hyprland, Wayland/wlroots, Waybar activation,
floating/manual drag, or physical hardware validation. Version `0.95.0` adds architecture/backend/protocol contract
diagnostics to `desktop architecture`, `desktop backend`, `desktop protocol`,
JSON `architecture|backend|protocol`, and generated package metadata:
`single-framebuffer-surface-v0`, `software-raster-present-v0`,
`internal-tiled-client-v0`, backend capabilities/limits, protocol limits, and a
future backend contract for replacing the VM framebuffer backend later without
claiming Wayland today. Version `0.94.0` adds modular desktop split diagnostics to
...
state without enabling Waybar, taskbar/start menu, floating windows, or manual
drag. Version `0.92.0` adds active submap role/action/sticky reset
...
diagnostics to config apply/doctor/configerrors and
`desktop hyprctl -j configerrors`, including loaded/missing/skipped counts,
depth/file limits, and status entries for each safe `source` file. Version
`0.86.0` adds explicit bind flag diagnostics to
`desktop configerrors`, `desktop hyprctl -j configerrors`,
`desktop hyprctl -j binds`, and `desktop hyprctl -j shortcuts`, including
plain/keyboard/mouse/composite counts and `manualDragFromBindm=false`.
Version `0.85.0` adds structured `error`/`hint` fields to
...
adds `desktop architecture` and structured
...
`desktop hyprctl -j shortcuts` diagnostics for keyboard-only navigation,
submaps, dispatcher-backed workspace/client actions, optional
focus-follows-mouse state, and the `manualDrag=false` plus
`bindmPreparedOnly=true` boundary. Version `0.78.0` adds structured
`desktop hyprctl -j autostart` and `desktop hyprctl -j autostart terminal
on|off|toggle` diagnostics/actions with persisted terminal autostart state,
generated `exec-once` runtime hints, session/runtime paths, live-installed
policy boundaries, and explicit `manualDrag=false`, `taskbar=false`,
`startMenu=false`, and `waybarActive=false` limits. Version `0.77.0` adds structured
`desktop hyprctl -j apps`, `desktop hyprctl -j app <id>`, and
`desktop hyprctl -j launch <app>` native app diagnostics/actions with
tiling-client counts, app class/module/source/runbook metadata, launch result,
and explicit launcher overlay, `manualDrag=false`, `taskbar=false`,
`startMenu=false`, and `waybarActive=false` boundaries. Version `0.76.0` adds structured
`desktop hyprctl -j session [status|start|stop|restart|reload|recover|rescue]`
session manager diagnostics/actions with health, desired/runtime state,
lifecycle counters, state/log paths, and a session log tail. It remains
VM/ZimaOS diagnostics only and does not add Waybar, a taskbar/start menu,
floating windows, free mouse drag, upstream Hyprland IPC compatibility,
Wayland/wlroots, or physical hardware validation. Version `0.75.0` adds structured
`desktop hyprctl -j dispatch <dispatcher> [args]` action output with
dispatcher/argument/result fields plus active workspace/layout/submap and
focused-client metadata. It wraps the existing tiled dispatcher engine only;
it does not add Waybar, a taskbar/start menu, floating windows, free mouse
drag, upstream Hyprland IPC compatibility, Wayland/wlroots, or physical
hardware validation. Version `0.74.0` extends the JSON shell/diagnostic layer with
`desktop hyprctl -j layouts`, `desktop hyprctl -j descriptions`,
`desktop hyprctl -j instances`, `desktop hyprctl -j modules`,
`desktop hyprctl -j shortcuts`,
`desktop hyprctl -j submap`,
