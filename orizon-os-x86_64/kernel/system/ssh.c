...
static void ssh_shell_print_desktop(const char *args) {
  ...
  if (*sub == '\0' || ssh_shell_command_is(sub, "status")) {
    ...
    snprintf(out, sizeof(out),
             ...
             "  desktop truth           show VM/facade/prepared/not-implemented taxonomy\r\n"
             ...
             "  desktop dispatch <d>    run exec/workspace/layoutmsg/master/swapwindow/movewindow/submap\r\n"
             ...
             "  desktop dispatch togglespecialworkspace [name] toggle tiled special scratchpad\r\n"
             "  desktop dispatch renameworkspace <target> <name> rename a workspace\r\n"
             "  desktop dispatch movetoworkspace <target|special[:name]>[,<window>] move focused/selected client and follow\r\n"
             "  desktop dispatch movetoworkspacesilent <target|special[:name]>[,<window>] move focused/selected client without switching\r\n"
             "  desktop dispatch fullscreen|fullscreenstate|pseudo|pseudotile|pin [state] set client state\r\n"
             "  desktop dispatch focuscurrentorlast|focusurgentorlast|markurgent|tagwindow focus history, urgent and tag diagnostics\r\n"
             "  desktop dispatch cyclenext|swapnext|swapwindow|movewindow|focusmaster|swapwithmaster client actions\r\n"
             "  desktop dispatch movewindow <l|r|u|d|next|prev|master> reorder tiled client without drag\r\n"
             "  desktop dispatch movefocus <l|r|u|d|next|prev> directional tiled focus\r\n"
             "  desktop dispatch focuswindow <id|0xaddr|class:app|title:text|tag:name|activewindow> focus matching client\r\n"
  ...
  } else if (ssh_shell_command_is(sub, "truth") ||
             ssh_shell_command_is(sub, "taxonomy") ||
             ssh_shell_command_is(sub, "limits")) {
    orizon_desktop_format_truth(out, sizeof(out));
    ...
    if (*hypr == '\0' || ssh_shell_command_is(hypr, "help")) {
    ...
    } else if (ssh_shell_command_is(hypr, "truth") ||
               ssh_shell_command_is(hypr, "taxonomy") ||
               ssh_shell_command_is(hypr, "limits")) {
      if (json) {
        orizon_desktop_format_truth_json(out, sizeof(out));
      } else {
        orizon_desktop_format_truth(out, sizeof(out));
      }
      ...
      if (json) {
        gui_desktop_format_monitors_json(out, sizeof(out));
      } else {
      ...
      if (json) {
        gui_desktop_format_layouts_json(out, sizeof(out));
      } else {
        gui_desktop_format_layouts(out, sizeof(out));
      }
      ...
      if (json) {
        gui_desktop_format_animations_json(out, sizeof(out));
      } else {
        gui_desktop_format_animations(out, sizeof(out));
      }
    } else if (ssh_shell_command_is(hypr, "decorations")) {
      if (json) {
        gui_desktop_format_decorations_json(out, sizeof(out));
      } else {
        gui_desktop_format_decorations(out, sizeof(out));
      }
      ...
      if (json) {
        gui_desktop_format_render_json(out, sizeof(out));
      } else {
        gui_desktop_format_render(out, sizeof(out));
      }
    } else if (ssh_shell_command_is(hypr, "descriptions")) {
      if (json) {
        gui_desktop_format_descriptions_json(out, sizeof(out));
      } else {
        gui_desktop_format_descriptions(out, sizeof(out));
      }
    ...
    } else if (ssh_shell_command_is(hypr, "modules") ||
               ssh_shell_command_is(hypr, "modulemap") ||
               ssh_shell_command_is(hypr, "module-map")) {
      if (json) {
        orizon_desktop_format_modules_json(out, sizeof(out));
      } else {
        orizon_desktop_format_modules(out, sizeof(out));
      }
      ...
      if (json) {
        ...
        target[target_len] = '\0';
        value = ssh_shell_skip_spaces(value + target_len);
        orizon_desktop_autostart_manager_json(target, value, out, sizeof(out));
        if (strcmp(target, "terminal") == 0 && *value &&
            !ssh_shell_command_is(value, "status") &&
            !ssh_shell_command_is(value, "show") &&
            !ssh_shell_command_is(value, "state")) {
          gui_desktop_reload_session();
        }
      } else if (*auto_args == '\0' ||
                 ssh_shell_command_is(auto_args, "status") ||
                 ssh_shell_command_is(auto_args, "show") ||
                 ssh_shell_command_is(auto_args, "state")) {
      ...
      if (*value == '\0' || ssh_shell_command_is(value, "show") ||
          ssh_shell_command_is(value, "status")) {
        if (json) {
          gui_desktop_format_submap_json(out, sizeof(out));
        } else {
          gui_desktop_format_submap(out, sizeof(out));
        }
      } else {
      ...
      if (json) {
        gui_desktop_format_splash_json(out, sizeof(out));
      } else {
