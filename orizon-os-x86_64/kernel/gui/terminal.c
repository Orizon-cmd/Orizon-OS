...
static void term_run_desktop(terminal_t *term, const char *cmd) {
  ...
  if (term_command_is(args, "help")) {
    ...
    term_puts_t(term, "  desktop truth           - Show VM/facade/prepared/not-implemented taxonomy\n");
    ...
    term_puts_t(term, "  desktop dispatch togglespecialworkspace [name] - Toggle tiled special scratchpad\n");
    term_puts_t(term, "  desktop dispatch renameworkspace <target> <name> - Rename a workspace\n");
    term_puts_t(term, "  desktop dispatch movetoworkspace <target|special[:name]>[,<window>] - Move focused/selected client and follow\n");
  ...
  if (term_command_is(args, "truth") ||
      term_command_is(args, "taxonomy") ||
      term_command_is(args, "limits")) {
    orizon_desktop_format_truth(report, sizeof(report));
    term_puts_t(term, report);
    return;
  }
  ...
  if (term_command_is(args, "hyprctl")) {
    ...
    if (term_command_is(hypr, "version")) {
    ...
    } else if (term_command_is(hypr, "truth") ||
               term_command_is(hypr, "taxonomy") ||
               term_command_is(hypr, "limits")) {
      if (json) {
        orizon_desktop_format_truth_json(report, sizeof(report));
      } else {
        orizon_desktop_format_truth(report, sizeof(report));
      }
    ...
    } else if (term_command_is(hypr, "modules") ||
               term_command_is(hypr, "modulemap") ||
               term_command_is(hypr, "module-map")) {
      if (json) {
        orizon_desktop_format_modules_json(report, sizeof(report));
      } else {
        orizon_desktop_format_modules(report, sizeof(report));
      }
    } else if (term_command_is(hypr, "shortcuts") ||
               term_command_is(hypr, "shortcutmap") ||
               term_command_is(hypr, "keys")) {
      if (json) {
        orizon_desktop_format_shortcuts_json(report, sizeof(report));
      } else {
        orizon_desktop_format_shortcuts(report, sizeof(report));
      }
      ...
      const char *app =
          term_skip_spaces(hypr + (term_command_is(hypr, "apps") ? 4 : 7));
      if (*app) {
        if (json) {
          orizon_desktop_format_app_detail_json(app, report, sizeof(report));
        } else {
          orizon_desktop_format_app_detail(app, report, sizeof(report));
        }
      } else if (json) {
        orizon_desktop_format_apps_json(report, sizeof(report));
      } else {
        orizon_desktop_format_apps(report, sizeof(report));
      }
    } else if (term_command_is(hypr, "app")) {
      const char *app = term_skip_spaces(hypr + 3);
      if (*app == '\0') {
        if (json) {
          orizon_desktop_format_app_detail_json("", report, sizeof(report));
        } else {
          snprintf(report, sizeof(report),
    ...
    } else if (term_command_is(hypr, "launch") ||
               term_command_is(hypr, "exec")) {
      const char *app = term_skip_spaces(
          hypr + (term_command_is(hypr, "launch") ? 6 : 4));
      char launch_result[256];
      int rc;
      if (*app == '\0') {
        if (json) {
          orizon_desktop_format_app_launch_json(
              "", "usage: desktop hyprctl launch <app>", -1, report,
              sizeof(report));
        } else {
          snprintf(report, sizeof(report),
        ...
        if (json) {
          orizon_desktop_format_app_launch_json(app, launch_result, rc, report,
                                                sizeof(report));
        } else {
          snprintf(report, sizeof(report), "%s", launch_result);
        }
      }
