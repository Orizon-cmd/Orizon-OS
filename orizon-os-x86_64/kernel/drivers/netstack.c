...
typedef struct {
  ...
  int root_trust_checked;
  int root_trust_verified;
  const char *root_trust_status;
...
static const char tls_root_isrg_x1_subject_sha256[] =
    "bd4d0c449b6e9aa74dce4b9d68b0cd54c042bc67e6ffedc2ecf2bf8479c951d4";

static const uint8_t tls_root_isrg_x1_n[512] = {
...
static int tls_verify_rsa_sha256_cert_signature(const tls_cert_summary_t *cert,
                                                const uint8_t *modulus,
                                                size_t modulus_len,
                                                uint32_t exponent) {
  uint8_t encoded[TLS_RSA_MAX_BYTES];

  if (!cert || !modulus || modulus_len == 0 || modulus_len > TLS_RSA_MAX_BYTES ||
      exponent != 65537 || !cert->signature_sha256_rsa ||
      !cert->signature_alg_consistent || !cert->signature_ready ||
      cert->signature_len > modulus_len) {
    return -1;
  }
  if (rsa_modexp65537(encoded, cert->signature_bytes, cert->signature_len,
                      modulus, modulus_len) != 0) {
    return -1;
  }
  return tls_check_pkcs1_sha256(encoded, modulus_len, cert->tbs_digest) ? 0
                                                                       : -1;
}
...
static int tls_verify_leaf_signature(tls_parse_info_t *info) {
  ...
  leaf = &info->certs[0];
  issuer = &info->certs[1];
  if (!issuer->public_key_rsa || !issuer->rsa_modulus_ready ||
      issuer->rsa_exponent != 65537 || issuer->rsa_modulus_len == 0) {
    info->leaf_signature_verify_status = "unsupported";
    return 0;
  }

  info->leaf_signature_verify_ready = 1;
  if (tls_verify_rsa_sha256_cert_signature(leaf, issuer->rsa_modulus,
                                           issuer->rsa_modulus_len,
                                           issuer->rsa_exponent) == 0) {
...
static int tls_verify_root_trust(tls_parse_info_t *info) {
  tls_cert_summary_t *issuer;
  ...
  issuer = &info->certs[1];
  info->root_trust_checked = 1;
  if (strcmp(issuer->issuer_sha256, tls_root_isrg_x1_subject_sha256) != 0) {
    info->root_trust_status = "untrusted-root";
    return 0;
  }

  if (tls_verify_rsa_sha256_cert_signature(
          issuer, tls_root_isrg_x1_n, sizeof(tls_root_isrg_x1_n), 65537) !=
      0) {
    info->root_trust_status = "bad-root-signature";
    return 0;
  }

  info->root_trust_verified = 1;
  info->root_trust_status = "isrg-root-x1";
  return 1;
}
  ...
  if (info.certificate_seen) {
    ...
    if (info.cert_summaries > 0) {
      ...
      if (info.cert_summaries > 1) {
        ...
        append_text(out, out_cap, line);
        snprintf(line, sizeof(line),
                 "tls root-trust checked=%s verified=%s anchor=%s\n",
                 info.root_trust_checked ? "yes" : "no",
                 info.root_trust_verified ? "yes" : "no",
                 info.root_trust_status ? info.root_trust_status : "not-run");
        append_text(out, out_cap, line);
      }
    }
  }
...
static const char *tls_policy_failure(const tls_parse_info_t *info) {
  if (!info || !info->certificate_seen) {
    return "tls certificate missing";
  }
  if (!info->leaf_identity_parsed || !info->leaf_identity_match) {
    return "tls host identity mismatch";
  }
  if (info->chain_links_checked == 0 ||
      info->chain_links_ok != info->chain_links_checked) {
    return "tls certificate chain link mismatch";
  }
  if (!info->leaf_signature_verified) {
    return "tls leaf signature not verified";
  }
  if (!info->root_trust_verified) {
    return "tls root trust not verified";
  }
  return NULL;
}
...
int netstack_https_range_get(const char *host, const char *path,
  ...
  tls_parse_records(tls_rx_buf, rx_len, &send_info, host);
  {
    const char *tls_policy_error = tls_policy_failure(&send_info);
    if (tls_policy_error) {
      return https_range_error(diag, diag_cap, -18, tls_policy_error);
    }
  }
  if (!send_info.client_key_exchange_ready ||
      send_info.client_key_exchange_record_len == 0) {
